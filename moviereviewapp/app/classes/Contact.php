...
class Contact {
    ...
    public function submitMessage($name, $email, $subject, $message) {
        ...
        $status = 'unread';
        $stmt = $this->db->prepare('INSERT INTO contact_messages (name, email, subject, message, status) VALUES (?, ?, ?, ?, ?)');
    ...
    public function replyToMessage($id, $reply_message, $replied_by) {
        ...
        $status = 'replied';
        $stmt = $this->db->prepare('UPDATE contact_messages SET status = ?, reply_message = ?, replied_by = ?, replied_at = NOW() WHERE id = ?');
    ...
    public function getUnreadCount() {
        $result = $this->db->query('SELECT COUNT(*) as total FROM contact_messages WHERE status = "unread"');
        $row = $result->fetch_assoc();
        return $row['total'];
    }

    public function getMessagesByStatus($status, $limit = 10, $offset = 0) {
        $stmt = $this->db->prepare('SELECT * FROM contact_messages WHERE status = ? ORDER BY created_at DESC LIMIT ? OFFSET ?');
    ...
    public function getTotalCount() {
        $result = $this->db->query('SELECT COUNT(*) as total FROM contact_messages');
        $row = $result->fetch_assoc();
        return $row['total'];
    }
}
