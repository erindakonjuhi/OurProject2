...
class Movie {
    ...
    public function addMovie($title, $description, $director, $release_year, $genre, $poster_image, $rating, $created_by) {
        ...
        // Correct order: s=string, i=integer, d=double
        $stmt->bind_param('sssiisdi', $title, $description, $director, $release_year, $genre, $poster_image, $rating, $created_by);

        if ($stmt->execute()) {
            return ['success' => true, 'movie_id' => $this->db->insert_id, 'message' => 'Movie added successfully'];
        } else {
            return ['success' => false, 'errors' => ['Failed to add movie: ' . $stmt->error]];
        }
    }
    ...
    public function updateMovie($id, $title, $description, $director, $release_year, $genre, $poster_image, $rating, $updated_by) {
        $rating = floatval($rating);
        $release_year = intval($release_year);
        $updated_by = intval($updated_by);
        $id = intval($id);
        
        $stmt = $this->db->prepare('UPDATE movies SET title = ?, description = ?, director = ?, release_year = ?, genre = ?, poster_image = ?, rating = ?, updated_by = ?, updated_at = NOW() WHERE id = ?');
        
        $stmt->bind_param('sssiissii', $title, $description, $director, $release_year, $genre, $poster_image, $rating, $updated_by, $id);
